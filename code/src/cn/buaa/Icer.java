...
public class Icer implements Runnable{
    //用于表示横纵坐标
    private int x;
    private int y;
    //用于表示当前的状态
    ...
    private int numOfBlueDiamond=0;
    ...
    public Icer (int x, int y) {
        this.x = x;
        this.y = y;
        show = StaticValue.icer_stand;
        this.status = "stop";
...
//                System.out.println("还在找");
            ...
            //判断是否在火河或泥沼中
            boolean isFireRiver = false;
            boolean isMudRiver = false;
            for (int i = 0; i < backGround.getFire_river_List().size(); i++) {
                Prop pr = backGround.getFire_river_List().get(i);
                //判断冰人是否位于火河上
...
//                System.out.println("火人"+this.x);
            ...
            //冰人死亡
            if(isFireRiver || isMudRiver) {
                death();
            }

            //冰人捡到蓝钻
            for (int i = 0;i < backGround.getBlueDiamondList().size();i++) {
                Prop pr = backGround.getBlueDiamondList().get(i);
                //判断捡到钻石
            ...
            //冰人到冰门
            for (int i = 0; i < backGround.getIceGateList2().size(); i++) {
                Prop pr = backGround.getIceGateList2().get(i);
                if ((pr.getY()-35 <= this.y && this.y <= pr.getY()) && (pr.getX() - 40 <= this.x && this.x <= pr.getX() + 35)) {
                    pr.arriveBlueGate();
                }
            }
...
//                    System.out.println("?");
            ...
            if ("move--left".equals(status)) {
                show = StaticValue.icer_run_L;
            }
            //判断是否向右移动
            if ("move--right".equals(status)) {
                show = StaticValue.icer_run_R;
            }
            //判断是否停止
            ...
            if("jump--left--up".equals(status)) {
                show = StaticValue.icer_jump_up_L;
            }
            //判断是否向左下跳跃
            if("jump--left".equals(status)) {
                show = StaticValue.icer_jump_down_L;
            }
            //判断是否向右上跳跃
            if("jump--right".equals(status)) {
                show = StaticValue.icer_jump_up_R;
            }
            //判断是否向右下跳跃
            if("jump--right".equals(status)) {
                show = StaticValue.icer_jump_down_R;
            }
            //判断是否向上跳跃
            if("jump--right".equals(status)) {
                show = StaticValue.icer_jump_up;
            }
            //判断是否向下跳跃
            if("jump--right".equals(status)) {
                show = StaticValue.icer_jump_down;
            }
    ...
    public int getNumOfBlueDiamond() {
        return numOfBlueDiamond;
    }
}
